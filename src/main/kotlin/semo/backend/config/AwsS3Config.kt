...
class AwsS3Config {
    ...
    @ConditionalOnExpression(
        "'\${app.aws.access-key-id:}' != '' && '\${app.aws.secret-access-key:}' != '' && '\${app.aws.region:}' != '' && ('\${app.aws.s3-bucket:}' != '' || '\${app.aws.profile-pictures-bucket:}' != '')",
    )
    fun s3Client(awsS3Properties: AwsS3Properties): S3Client {
        ...
        if (awsS3Properties.s3Bucket.isNotBlank()) {
            requireNoUnresolvedPlaceholder(awsS3Properties.s3Bucket, "AWS_S3_BUCKET")
        }
        requireNoUnresolvedPlaceholder(awsS3Properties.profilePicturesBucket, "AWS_PROFILE_PICTURES_BUCKET")
